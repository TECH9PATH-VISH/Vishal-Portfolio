document.addEventListener('DOMContentLoaded', () => {
    ...
    */
    ...
    // ============================================================
    // FEATURE 9 — CUSTOM GITHUB ACTIVITY HEATMAP
    // ============================================================
    const ghGrid       = document.getElementById('gh-grid');
    const ghMonthRow   = document.getElementById('gh-month-labels');
    const ghTotalLabel = document.getElementById('gh-total-label');
    const ghSection    = document.getElementById('github-activity');

    const MONTHS = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
    const CELL_W = 14; // px per column (cell + gap)
    ...
    function buildHeatmap(days) {
        if (!ghGrid || !ghMonthRow) return;
        ghGrid.innerHTML = '';
        ghMonthRow.innerHTML = '';
        ...
        // Padding cells so the first real day lands in the right column
        for (let i = 0; i < startDow; i++) {
            const empty = document.createElement('div');
            empty.className = 'gh-cell gh-level-0';
            ghGrid.appendChild(empty);
        }
        ...
        days.forEach((d, idx) => {
            ...
            const level = d.level !== undefined ? d.level : getLevel(count);
            ...
            cell.setAttribute('data-tip', `${count} contribution${count !== 1 ? 's' : ''} on ${dateStr}`);

            const colDelay = Math.floor((startDow + idx) / 7) * 0.008;
            cell.style.opacity = '0';
            cell.style.transition = `opacity 0.35s ease ${colDelay.toFixed(3)}s`;
            ghGrid.appendChild(cell);
        });

        // Animate cells in on next frame
        requestAnimationFrame(() => requestAnimationFrame(() => {
            ghGrid.querySelectorAll('.gh-cell').forEach(c => { c.style.opacity = '1'; });
        }));

        // Build month label spans
        const totalCols = Math.ceil((startDow + days.length) / 7);
        monthPositions.forEach(({ month, col }, i) => {
            const nextCol  = monthPositions[i + 1]?.col ?? totalCols;
            const spanCols = Math.max(nextCol - col, 1);
            const label    = document.createElement('span');
            label.className = 'gh-month-label';
            label.textContent = MONTHS[month];
            label.style.width = (spanCols * CELL_W) + 'px';
            ghMonthRow.appendChild(label);
        });

        if (ghTotalLabel) {
            ghTotalLabel.textContent = `${total.toLocaleString()} contributions in the last year`;
        }
    }
    ...
    function generateFallbackData() {
        ...
        for (let i = 364; i >= 0; i--) {
            ...
            if (dow > 0 && dow < 6) {
                const r = rand();
                if (r > 0.55) count = Math.floor(rand() * 8) + 1;
                if (r > 0.85) count = Math.floor(rand() * 14) + 5;
            } else {
                if (rand() > 0.78) count = Math.floor(rand() * 4) + 1;
            }
            days.push({ date: d.toISOString().split('T')[0], count, level: getLevel(count) });
        }
        return days;
    }
    ...
    function fetchGitHubActivity() {
        if (!ghGrid) return;
        ghGrid.innerHTML = '<div class="gh-loading"><div class="gh-loading-spinner"></div><span>Loading activity...</span></div>';

        fetch('https://github-contributions-api.jogruber.de/v4/TECH9PATH-VISH?y=last', {
            signal: AbortSignal.timeout(8000)
        })
        .then(r => r.json())
        .then(data => {
            if (data && data.contributions && data.contributions.length > 0) {
                buildHeatmap(data.contributions);
            } else {
                buildHeatmap(generateFallbackData());
            }
        })
        .catch(() => buildHeatmap(generateFallbackData()));
    }
